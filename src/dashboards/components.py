    ...
    for i, (name, values) in enumerate(y_data.items()):
        fig.add_trace(
            go.Scatter(
                ...
                name=name,
                mode="lines",
                line={"color": sensor_colors[i % len(sensor_colors)], "width": 2},
            )
        )
    fig.update_layout(
        title={"text": title, "font": {"color": COLORS["text_primary"], "size": 16}},
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(0,0,0,0)",
        xaxis={"gridcolor": COLORS["border"], "color": COLORS["text_secondary"]},
        yaxis={"gridcolor": COLORS["border"], "color": COLORS["text_secondary"]},
...
def distribution_bar(labels: list[str], values: list[int], title: str = "") -> dcc.Graph:
    ...
    fig.update_layout(
        ...
        plot_bgcolor="rgba(0,0,0,0)",
        xaxis={"gridcolor": COLORS["border"], "color": COLORS["text_secondary"]},
        yaxis={"color": COLORS["text_secondary"]},
        height=250,
        margin={"t": 50, "b": 30, "l": 80, "r": 20},
    )
    return dcc.Graph(figure=fig, config={"displayModeBar": False})
